...
__powerline() {
    ...
    ps1() {
        ...
        # show any background jobs
        if [ -n "$(jobs -p)" ]; then 
            local CURRENT_JOBS="$BG_RED"  # job in background
        else
            local CURRENT_JOBS="$BG_BASE1"   # no jobs in background
        fi
        PS1="$BG_BASE1\u${CURRENT_JOBS}@$BG_BASE1\h $BG_CYAN \w $RESET" 
        ...
        if shopt -q promptvars; then
            ...
            PS1+="$BG_BLUE\${__powerline_git_info}$RESET"
        else
            # promptvars is disabled. Avoid creating unnecessary env var.
            PS1+="$BG_BLUE$(__git_info)$RESET"
        fi
